...
instanceIPv4() {
    ...
    for ifName in enp5s0 eth0; do
        ipv4=$(lxc ls "${instance}" -f csv -c 4 | grep -oP "(\d{1,3}\.){3}\d{1,3}(?= \(${ifName}\))" || true)
        if [ "${ipv4}" != "" ]; then
            echo "${ipv4}"
            return 0
        fi
...
EOF
    ...
    for i in $(seq 1 "${CLUSTER_SIZE}"); do
        ...
        local args=(--profile container-kvm --config "security.nesting=true")
        local ifName="eth0"
