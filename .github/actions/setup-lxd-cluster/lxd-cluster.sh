...
EOF
    ...
    for i in $(seq 1 "${CLUSTER_SIZE}"); do
        ...
        if [ "${INSTANCE_TYPE}" = "virtual-machine" ]; then
            ...
            # which unprivileged containers do not have. The ceph kernel module must
            # also be loaded on the host, as containers cannot load it themselves.
            args+=(--config security.privileged=true --config "linux.kernel_modules=kvm,vhost_net,vhost_vsock,ceph")
        fi
...
EOF
    ...
    for i in $(seq 1 "${CLUSTER_SIZE}"); do
        ...
        if [ "${INSTANCE_TYPE}" = "container" ] && [ "${MICROCEPH_ENABLED}" = "true" ]; then
            # Privileged containers cannot mount binfmt_misc, so the systemd-binfmt unit
            # fails and leaves the system degraded.
...
# configure_microceph installs and bootstraps MicroCeph on the dedicated instance and
# distributes its client configuration and keyring to every cluster member, so that
# the members can use Ceph-backed storage pools.
configure_microceph() {
    if [ "${MICROCEPH_ENABLED}" != "true" ]; then
    ...
    # The files are read from the MicroCeph snap directory, as symlinking it to /etc/ceph
